...
#EXPLORE KAPPA and Rho AND FIT ONLY ONE PARAMETER EPSILON*Ni: from tqdm.autonotebook import tqdm
...
for i in range(len(FillNumber)):
    ...
    params, pcov = curve_fit(L_model_3param, Turn,L_Tau,p0,bounds=([2e-10, 150., 0.7], [2e-9, 170., 1.2]),maxfev=5000)
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit0, "r-", markersize=4, label='2 Param Fit Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit, "r-", markersize=4, label='2 Param Fit Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    ...
    ax.set_title('$2$ $Parameter$ $Fit$ $for$ $FillNumber:$ ${}$'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    #plt.legend(loc='best')
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best1_L_Tau_fit0, "r-", markersize=4, label='1 param fit')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    ...
    subsets_2param.append((T_fit_real, L_Tau_fit1_last))             
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit1, "r-", markersize=4, label='1 Param Fit Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit1, "y-", markersize=4, label='1 Param Fit Model-2')
    ax.plot(T_fit_real/3600, best_L_Tau_fit, "r-", markersize=4, label='2 Param Fit Model-2')
    ax.plot(T_fit_real/3600, L, "g-", markersize=4, label='3 param Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ...
    for x in range(len(k1_val)):
        with open('20{}/Coeff/{}_1param_first_FitCoeff.txt'.format((str(year)),text), 'a') as f:
            f.write(str(k1_val[x]))
            f.write(' ')
            f.write(str(eps_ni1[x]))
            f.write(' ')
            f.write(str(B1_val[x]))
            f.write(' ')
            f.write(str(R_squared1[x]))
            f.write(' ')
            f.write(str(chi_squared1[x]))
            f.write('\n')

    for x in range(len(k1_val_last)):
        with open('20{}/Coeff/{}_1param_last_FitCoeff.txt'.format((str(year)),text), 'a') as f:
            f.write(str(k1_val_last[x]))
            f.write(' ')
            f.write(str(eps_ni1_last[x]))
            f.write(' ')
            f.write(str(B1_val_last[x]))
            f.write(' ')
            f.write(str(R_squared1_last[x]))
            f.write(' ')
            f.write(str(chi_squared1_last[x]))
            f.write('\n')
    ...
    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    #for i, (T_fit_real, L_Tau_fit_last) in enumerate(subsets_2param):
    for i in range(L_Tau_fit1_last.shape[0]):
        color = colors[i % len(colors)]
        #label = f'Subset [i % len(colors)]'
        #label = f'$\kappa$={k_values_last[i]}'
        label = f'k_values {i+1}'
        
        ax.plot(T_fit_real/3600, L_Tau_fit1_last[i,:], color=color, label=label)
            
    ax.set_title('$1$ $Parameter$ $Fit$ $for$ $FillNumber:$  ${}$'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    #plt.legend(loc='best')
