...
for i in range(len(FillNumber)):
    ...
    p_2param = (2.5e-10, 155)
    
    for i, k in enumerate(k_values):
        #p_2param = (2.5e-10, 155)
        
                
        popt, pcov = curve_fit(L_model_2param, Turn, L_Tau, p0=p_2param, bounds=([2e-10, 50], [2.e-9, 200.]), maxfev=5000)
        ...
        subsets2.append((T_fit_real, L_Tau_fit[i]))     
    ...
    for i, k in enumerate(k_values_last): 
        ...
        popt2, pcov2 = curve_fit(L_model_2param, Turn, L_Tau, p0=p_2param, bounds=([2e-10, 50], [2.e-9, 200.]), maxfev=5000)
        ...
        subsets_2param.append((T_fit_real, L_Tau_fit_last[i]))     
    ...
    for i, (T_fit_real, L_Tau_fit_last) in enumerate(subsets_2param):
    ...
    ##################################################### FITTING ONLY ONE PARAMETER ##########################################

    k1_min = 0.1
    k1_max = 2
    dk1 = 0.01
    #k_values = np.arange(k_min, k_max+dk, dk)
    k1_values = np.linspace(k1_min, k1_max, int((k1_max-k1_min)/dk1)+1)

    B1_min = 150
    B1_max = 170
    #num_values = len(k_values)
    #dB = (B_max - B_min) / (num_values - 1)  # Adjusted step size for B_values
    dB1 = 1
    # Adjust the range and step size of B_values
    #B_values = np.arange(B_min, B_max + dB, dB)[:len(k_values)]
    B1_values = np.linspace(B1_min, B1_max, int((B1_max-B1_min)/dB1)+1)

    #k_arr = np.linspace(k_min, k_max, int((k_max-k_min)/dk)+1)
    L_Tau_fit1 = np.zeros((len(k1_values), len(B1_values), len(Turn)))

    eps_ni1 = np.zeros((len(k1_values),len(B1_values)))
    R_squared1 = np.zeros((len(k1_values),len(B1_values)))
    chi_squared1 = np.zeros((len(k1_values),len(B1_values)))
    
    
    min_chi_squared1 = np.inf
    min_chi_squared_index1 = None

    p_1param = (2.5e-10)
    
    for i, k in enumerate(k1_values):
        #p0 = (2.5e-10)
        for j, B in enumerate(B1_values):
        
            popt1, pcov1 = curve_fit(L_model_1param, Turn, L_Tau, p0=p_1param, bounds=([2e-10], [2e-9]), maxfev=5000)
            L_Tau_fit1[i, j] = L_model_1param(Turn, *popt1)
            eps_ni1[i, j] = popt1[0]


            residuals1 = L_Tau - L_model_1param(Turn, *popt1)
            ss_res1 = np.sum(residuals**2)
            ss_tot1 = np.sum((L_Tau - np.mean(L_Tau))**2)

            subsets1.append((T_fit_real, L_Tau_fit1[i, j]))  
    
            if ss_tot1 == 0:
                r_2 = 1
            else:
                r_2 = 1 - (ss_res1 / ss_tot1)     
            ...
            p1 = len(popt1) - 1
            # calculate the adjusted R-squared value
            R_squared1[i,j] = 1 - (1 - r_2) * (n1 - 1) / (n1 - p1 - 1)
            chi_squared1[i,j] = np.sum(((residuals1)**2)/L_Tau)
            
            
            print('FillNumber', text,'R_squared', R_squared1)
            print('popt is',popt1) 
            print('iteration i -- j is',i,j) 
            #p0 = popt
                    
            if chi_squared1[i,j] < min_chi_squared1:
                min_chi_squared1 = chi_squared1[i,j]
                min_chi_squared_index1 = (i,j)
            
            print('min_chi_squared is',min_chi_squared) 


    best1_k0 = k1_values[min_chi_squared_index1[0]]
    k = best1_k0
    best1_B0 = B1_values[min_chi_squared_index1[1]]
    B = best1_B0
    best1_eps_ni0 = eps_ni1[min_chi_squared_index1[0], min_chi_squared_index1[1]]
    best1_R_squared0 = R_squared1[min_chi_squared_index1[0], min_chi_squared_index1[1]]
    best1_chi_squared0 = chi_squared1[min_chi_squared_index1[0], min_chi_squared_index1[1]]
    best1_L_Tau_fit0 = L_model_1param(Turn, best1_eps_ni0)

    print('i ---- best1_k0 -- best1_B0 -- best1_eps_n0 is',i, best1_k0, best1_B0, best1_eps_ni0 )
    print('min_chi_squared1 and min_chi_squared_index1 is',min_chi_squared_index1, min_chi_squared1)

    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best1_L_Tau_fit0, "r-", markersize=4, label='1 param full interval')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    plt.savefig('20{}/best_mod/first_check/{}_1param_first_interval_Fit.pdf'.format(str(year),text))
        


    with open('20{}/best_param/20{}_1param_first_best_FitCoeff.txt'.format(str(year),str(year)), 'a') as f:
        f.write(text)
        f.write(' ')
    ...
    dk1_last = 0.001
    #k_values_last = np.arange(k_min_last, k_max_last+dk_last, dk_last)
    k1_values_last = np.linspace(k1_min_last, k1_max_last, int((k1_max_last-k1_min_last)/dk1_last)+1)

    B1_min_last = best_B0 - 0.9
    B1_max_last = best_B0 + 0.9
    dB1_last = 0.1
    B1_values_last = np.linspace(B1_min_last, B1_max_last, int((B1_max_last-B1_min_last)/dB1_last)+1)
    
    L_Tau_fit1_last = np.zeros((len(k1_values_last), len(B1_values_last),len(Turn)))   
    
    eps_ni1_last = np.zeros((len(k1_values_last), len(B1_values_last)))
    R_squared1_last = np.zeros((len(k1_values_last), len(B1_values_last)))
    chi_squared1_last = np.zeros((len(k1_values_last), len(B1_values_last)))

    min_chi_squared1_last = np.inf
    min_chi_squared1_index_last = None
    ...
    p_1param = (best1_eps_ni0)
    
        
    for i, k in enumerate(k1_values_last): 
        #p0 = (best_eps_ni0)
        for j, B in enumerate(B1_values_last):

            param1, pcov1 = curve_fit(L_model_1param, Turn, L_Tau, p0=p_1param, bounds=([2e-10], [2.e-9]), maxfev=5000)
            L_Tau_fit1_last[i,j] = L_model_1param(Turn, *param1)
            eps_ni1_last[i,j] = param1[0]

            res1 = L_Tau - L_model_1param(Turn, *param1)
            ss_res2 = np.sum(res1**2)
            ss_tot2 = np.sum((L_Tau - np.mean(L_Tau))**2)

            subsets_1param.append((T_fit_real, L_Tau_fit1_last[i,j]))    
            ...
            R_squared1_last[i,j] = 1 - (1 - r2) * (n1 - 1) / (n1 - p1 - 1)
            chi_squared1_last[i,j] = np.sum(((res1)**2)/L_Tau)

            print('popt_last is',param1) 
            print('iteration i is',i)

            #min_chi_squared_index_last = np.unravel_index(np.argmin(chi_squared_last), chi_squared_last.shape)

            
            if chi_squared1_last[i,j] < min_chi_squared1_last:
                min_chi_squared1_last = chi_squared1_last[i,j]
                min_chi_squared1_index_last = (i,j)

            print('min_chi_squared1_last is',min_chi_squared1_last)       
        

    best_k1 = k1_values_last[min_chi_squared1_index_last[0]]
    k = best_k1
    best_B1 = B1_values_last[min_chi_squared1_index_last[1]]
    B = best_B1
    best_eps_ni1 = eps_ni1_last[min_chi_squared1_index_last[0],min_chi_squared1_index_last[1]]
    best_R_squared1 = R_squared1_last[min_chi_squared1_index_last[0],min_chi_squared1_index_last[1]]
    best_chi_squared1 = chi_squared1_last[min_chi_squared1_index_last[0],min_chi_squared1_index_last[1]]
    best_L_Tau_fit1 = L_model_1param(Turn, best_eps_ni1)

    print('i_last ---- best_k1 -- best_B1 -- best_eps_n1 is', i, best_k1, best_B1, best_eps_ni1 )
    print('min_chi_squared_last and min_chi_squared_index_last is', min_chi_squared1_index_last, min_chi_squared1_last)

    with open('20{}/best_param/20{}_1param_last_best_FitCoeff.txt'.format(str(year),str(year)), 'a') as f:
        f.write(text)
        f.write(' ')
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit1, "r-", markersize=4, label='1 param Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    #plt.savefig('20{}/best_mod/{}_reduced_interval_Fit_mod2_best.pdf'.format(str(year),text))
    plt.savefig('20{}/best_mod/{}_1param_Fit.pdf'.format(str(year),text))
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit1, "y-", markersize=4, label='1 param Model-2')
    ax.plot(T_fit_real/3600, best_L_Tau_fit, "r-", markersize=4, label='2 param Model-2')
    ax.plot(T_fit_real/3600, L, "g-", markersize=4, label='3 param Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ...
    for x in range(len(k_values)):
        for y in range(len(B1_values)):
            with open('20{}/Coeff/{}_1param_first_FitCoeff.txt'.format((str(year)),text), 'a') as f:
                f.write(str(k1_values[x]))
                f.write(' ')
                f.write(str(eps_ni1[x,y]))
                f.write(' ')
                f.write(str(B1_values[y]))
                f.write(' ')
                f.write(str(R_squared1[x,y]))
                f.write(' ')
                f.write(str(chi_squared1[x,y]))
                f.write('\n')

    for x in range(len(k_values_last)):
        for y in range(len(B1_values_last)):
            with open('20{}/Coeff/{}_1param_last_FitCoeff.txt'.format((str(year)),text), 'a') as f:
                f.write(str(k1_values_last[x]))
                f.write(' ')
                f.write(str(eps_ni1_last[x,y]))
                f.write(' ')
                f.write(str(B1_values_last[y]))
                f.write(' ')
                f.write(str(R_squared1_last[x,y]))
                f.write(' ')
                f.write(str(chi_squared1_last[x,y]))
                f.write('\n')
            
            
    for x in range(len(T_fit_real)):
        with open('20{}/data/{}_1param_Fitdata.txt'.format((str(year)),text), 'a') as f:  
            f.write(str(T_fit_real[x]))
            f.write(' ')
            f.write(str(L_Tau[x]))
            f.write(' ')
            f.write(str(best_L_Tau_fit1[x]))
            f.write('\n')
    ...
    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    for i, (T_fit_real, L_Tau_fit1_last) in enumerate(subsets_1param):
        color = colors[i % len(colors)]
        #label = f'Subset [i % len(colors)]'
        #label = f'$\kappa$={k_values_last[i]}'
        label = f'k_values {i+1}'
        
        ax.plot(T_fit_real/3600, L_Tau_fit1_last, color=color, label=label)
            
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    #plt.legend(loc='best')
    plt.savefig('20{}/mod/{}_Fit_1param_scan.pdf'.format(str(year),text))
    #plt.show()


    delta_k1 = (best_k1 - k_all).tolist()
    delta_B1 = (best_B1 - B_all).tolist()
    delta_eps_ni1 = (best_eps_ni1 - eps_all).tolist()
    delta_R_squared1 = (best_R_squared1 - R_adj).tolist()
    delta_chi_squared1 = (best_chi_squared1 - chi_square).tolist()

    with open('20{}/best_param/20{}_delta_1param_Coeff.txt'.format(str(year),str(year)), 'a') as f:
        f.write(text)
        f.write(' ')
        f.write(str(delta_eps_ni1))
        f.write(' ')
        f.write(str(delta_B1))
        f.write(' ')
        f.write(str(delta_k1))
        f.write(' ')
        f.write(str(delta_R_squared1))
        f.write(' ')
        f.write(str(delta_chi_squared1))
        f.write('\n')
