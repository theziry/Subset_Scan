...
def fit_2paramxxx(Turn,L_Tau,k_min, k_max, k_samples):
    ...
    for k in enumerate(k_values):
        par, co_par = curve_fit(L_model_3param,
            Turn,
            L_Tau,
            p0=(2.5e-10, 155),
            bounds=([2e-10, 150], [2.e-9, 200.]),
            maxfev=5000
        )
        ...
        n = len(L_Tau)
        p = len(par) - 1
        r2 = R_2(L_Tau, L_Tau_fit)
        Radj = (1 - (1 - r2) * (n - 1) / (n - p - 1))
        Radj_pars.append(Radj)  

    return np.array(L_Tau_fit), np.array(pars), np.array(errs), np.array(co_pars), np.array(Radj_pars)
...
for i in range(len(FillNumber)):
    ...
    params, pcov = curve_fit(L_model_3param, Turn,L_Tau,p0,bounds=([2e-10, 150., 0.7], [2e-9, 200., 2]),maxfev=5000)
    ...
    #eps_ni.append(real_pars[0][0]) ##This will return only the first value in first column
    eps_ni=real_pars[:, 0]
    #B.append(real_pars[0][1]) ##This will return only the first value in second column
    B=real_pars[:,1]
    ...
    #[np.argmin(real_errs)]  best_k0 = k_values[min_err_index]
        
    #best_k0 = k_values[min_err_index]
    #best_B0 = B[0][min_err_index]
    #best_eps_ni0 = eps_ni[0][min_err_index]
    #best_R_squared0 = R_squared[0][min_err_index]
    #best_chi_squared0 = chi_squared[0][min_err_index]
    ...
    best_L_Tau_fit0 = L_model_3param(Turn, best_eps_ni0, best_B0,best_k0)
    #k = best_k0
    #best_L_Tau_fit0 = L_Tau_fit[0][min_err_index]
    ...
    #L_Tau_fit_last = real_L_Tau_fit_last.tolist()
    #L_Tau_fit_last.append(real_L_Tau_fit_last)
    ...
    #best_L_Tau_fit = L_Tau_fit_last[0][last_min_err_index]
