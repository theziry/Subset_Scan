...
for i in range(len(FillNumber)):
    ...
    B_values = np.linspace(B_min, B_max, int((B_max-B_min)/dB)+1)

    #k_arr = np.linspace(k_min, k_max, int((k_max-k_min)/dk)+1)
    L_Tau_fit = np.zeros((len(k_values), len(B_values), len(Turn)))

    eps_ni = np.zeros((len(k_values),len(B_values)))
    R_squared = np.zeros((len(k_values),len(B_values)))
    chi_squared = np.zeros((len(k_values),len(B_values)))
    ...
    for i, k in enumerate(k_values):
        ...
        for j, B in enumerate(B_values):
            ...
            popt, pcov = curve_fit(L_model_1param, Turn, L_Tau, p0=p0, bounds=([2e-10], [2e-9]), maxfev=5000)
            L_Tau_fit[i, j] = L_model_1param(Turn, *popt)
            eps_ni[i, j] = popt[0]


            residuals = L_Tau - L_model_1param(Turn, *popt)
            ss_res = np.sum(residuals**2)
            ss_tot = np.sum((L_Tau - np.mean(L_Tau))**2)

            subsets.append((T_fit_real, L_Tau_fit[i, j]))  
            ...
            p = len(popt) - 1
            # calculate the adjusted R-squared value
            R_squared[i,j] = 1 - (1 - r_2) * (n - 1) / (n - p - 1)
            chi_squared[i,j] = np.sum(((residuals)**2)/L_Tau)
            ...
            if chi_squared[i,j] < min_chi_squared:
                min_chi_squared = chi_squared[i,j]
                min_chi_squared_index = (i,j)
            
            print('min_chi_squared is',min_chi_squared) 


    best_k0 = k_values[min_chi_squared_index[0]]
    k = best_k0
    best_B0 = B_values[min_chi_squared_index[1]]
    B = best_B0
    best_eps_ni0 = eps_ni[min_chi_squared_index[0], min_chi_squared_index[1]]
    best_R_squared0 = R_squared[min_chi_squared_index[0], min_chi_squared_index[1]]
    best_chi_squared0 = chi_squared[min_chi_squared_index[0], min_chi_squared_index[1]]
    best_L_Tau_fit0 = L_model_1param(Turn, best_eps_ni0)
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit0, "r-", markersize=4, label='1 param full interval')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    ...
    B_min_last = best_B0 - 0.9
    B_max_last = best_B0 + 0.9
    dB_last = 0.1
    B_values_last = np.linspace(B_min_last, B_max_last, int((B_max_last-B_min_last)/dB_last)+1)
    
    L_Tau_fit_last = np.zeros((len(k_values_last), len(B_values_last),len(Turn)))   
    
    eps_ni_last = np.zeros((len(k_values_last), len(B_values_last)))
    R_squared_last = np.zeros((len(k_values_last), len(B_values_last)))
    chi_squared_last = np.zeros((len(k_values_last), len(B_values_last)))
    ...
    for i, k in enumerate(k_values_last): 
        #p0 = (best_eps_ni0)
        for j, B in enumerate(B_values_last):

            popt2, pcov2 = curve_fit(L_model_1param, Turn, L_Tau, p0=p0, bounds=([2e-10], [2.e-9]), maxfev=5000)
            L_Tau_fit_last[i,j] = L_model_1param(Turn, *popt2)
            eps_ni_last[i,j] = popt2[0]

            residuals2 = L_Tau - L_model_1param(Turn, *popt2)
            ss_res2 = np.sum(residuals2**2)
            ss_tot2 = np.sum((L_Tau - np.mean(L_Tau))**2)

            subsets2.append((T_fit_real, L_Tau_fit_last[i,j]))    
            ...
            R_squared_last[i,j] = 1 - (1 - r2) * (n - 1) / (n - p - 1)
            chi_squared_last[i,j] = np.sum(((residuals2)**2)/L_Tau)
            ...
            if chi_squared_last[i,j] < min_chi_squared_last:
                min_chi_squared_last = chi_squared_last[i,j]
                min_chi_squared_index_last = (i,j)

            print('min_chi_squared_last is',min_chi_squared_last)       
        

    best_k = k_values_last[min_chi_squared_index_last[0]]
    k = best_k
    best_B = B_values_last[min_chi_squared_index_last[1]]
    B = best_B
    best_eps_ni = eps_ni_last[min_chi_squared_index_last[0],min_chi_squared_index_last[1]]
    best_R_squared = R_squared_last[min_chi_squared_index_last[0],min_chi_squared_index_last[1]]
    best_chi_squared = chi_squared_last[min_chi_squared_index_last[0],min_chi_squared_index_last[1]]
    best_L_Tau_fit = L_model_1param(Turn, best_eps_ni)
    ...
    fig, ax = plt.subplots() 
    ax.plot(T_fit_real/3600, L_Tau, "b.", markersize=4, label='Data')
    ax.plot(T_fit_real/3600, best_L_Tau_fit, "r-", markersize=4, label='1 param Model-2')
    ax.set_title('FillNumber: {}'.format(text))
    ax.set_xlabel(' Time [h]')
    ax.set_ylabel('${L}/{L_i}$' ) 
    plt.legend(loc='best')
    ...
    for x in range(len(k_values)):
        for y in range(len(B_values)):
            with open('20{}/Coeff/{}_1param_first_FitCoeff.txt'.format((str(year)),text), 'a') as f:
                f.write(str(k_values[x]))
                f.write(' ')
                f.write(str(eps_ni[x,y]))
                f.write(' ')
                f.write(str(B_values[y]))
                f.write(' ')
                f.write(str(R_squared[x,y]))
                f.write(' ')
                f.write(str(chi_squared[x,y]))
                f.write('\n')

    for x in range(len(k_values_last)):
        for y in range(len(B_values_last)):
            with open('20{}/Coeff/{}_1param_last_FitCoeff.txt'.format((str(year)),text), 'a') as f:
                f.write(str(k_values_last[x]))
                f.write(' ')
                f.write(str(eps_ni_last[x,y]))
                f.write(' ')
                f.write(str(B_values_last[y]))
                f.write(' ')
                f.write(str(R_squared_last[x,y]))
                f.write(' ')
                f.write(str(chi_squared_last[x,y]))
                f.write('\n')
