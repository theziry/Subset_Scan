...
#EXPLORE THE KAPPA AND FIT ONLY TWO PARAMETER: from tqdm.autonotebook import tqdm
def fit_2param(Turn,L_Tau,k_min, k_max, k_samples):
    ...
    #return L_Tau_fit, B, eps_ni, chi_squared, co_param, R_squared
...
def fit_param(Turn,L_Tau,k_min, k_max, k_samples, B_min, B_max, B_samples):
    k_values = np.linspace(k_min, k_max, k_samples)
    B_values = np.linspace(B_min, B_max, B_samples)
    ...
    pars = np.zeros((len(k_values),len(B_values),1))
    errs = np.zeros((len(k_values),len(B_values)))
    co_pars = np.zeros((len(k_values),len(B_values),1,1))
    Radj_pars = np.zeros((len(k_values),len(B_values)))

    for i, k in enumerate(tqdm(k_values)):
        for j, B in enumerate(tqdm(B_values)):
            par, co_par = curve_fit(
                lambda x, eps_ni: L_model_3param(x,  eps_ni, B, k),
                Turn,
                L_Tau,
            ...
            p = len(par) - 1
            r2 = R_2(L_Tau, L_model_3param(Turn, par[0], B, k))
            L_Tau_fit[i, j]=(L_model_3param(Turn, par[0],B, k))
            #print('L_Tau --- ',L_Tau, '----- L_Tau_fit ------', L_Tau_fit)
            val 
            pars[i, j]=par
            co_pars[i, j]=co_par
            errs[i, j]=(chi_2(L_Tau, L_model_3param(Turn, par[0],B, k)))
            Radj = (1 - (1 - r2) * (n - 1) / (n - p - 1))
            Radj_pars[i, j]=Radj

    return L_Tau_fit, pars, errs, co_pars, Radj_pars
...
for i in range(len(FillNumber)):
    ...
    params, pcov = curve_fit(L_model_3param, Turn,L_Tau,p0,bounds=([2e-10, 50., 0.1], [2e-9, 200., 2]),maxfev=5000)
    ...
    B_max = 180
    dB = 1
    B_samples = (int((B_max-B_min)/dB)+1)
    B_values = np.linspace(B_min, B_max, B_samples)
    ...
    real_L_Tau_fit,real_k_values,real_B_values,real_pars,real_errs, real_co_pars, real_Radj_pars  = fit_1param(
        Turn,L_Tau,k_min, k_max, k_samples, B_min, B_max, B_samples
    )
    ...
    best_k0 = k_val[min_err_index]
    best_B0 = B_val[min_err_index]
    best_eps_ni0 = eps_ni[min_err_index]
    best_R_squared0 = R_squared[min_err_index]
    best_chi_squared0 = chi_squared[min_err_index]
    best_L_Tau_fit0 = L_model_3param(Turn, best_eps_ni0, best_B0,best_k0)
    ...
    dB_last = 0.1
    B_samples_last = (int((B_max_last-B_min_last)/dB_last)+1)
    B_values_last = np.linspace(B_min_last, B_max_last, B_samples_last)

    
    real_L_Tau_fit_last,real_k_values_last,real_B_values_last, real_pars_last, real_errs_last, real_co_pars_last, real_Radj_pars_last  = fit_1param(
        Turn,L_Tau,k_min_last, k_max_last, k_samples_last, B_min_last, B_max_last, B_samples_last
    )

    k_val_last=real_k_values_last
    B_val_last=real_B_values_last
    ...
    subsets_2param.append((T_fit_real, L_Tau_fit_last))             
    ...
    best_k = k_val_last[np.argmin(real_errs_last)]
    best_B = B_val_last[last_min_err_index]
    ...
    best_L_Tau_fit = L_model_3param(Turn, best_eps_ni, best_B,best_k)

    print('last_index ---- best_k -- best_kB -- best_eps_n is', i, last_min_err_index, best_k, best_B, best_eps_ni )
    print('min_chi_squared_last and min_chi_squared_index_last is', last_min_err_index, min_chi_squared_last, '---np.argmin(chi_squared_last--',np.argmin(chi_squared_last))
    ...
    for i in range(L_Tau_fit_last.shape[0]):
        color = colors[i % len(colors)]
        #label = f'Subset [i % len(colors)]'
        #label = f'$\kappa$={k_values_last[i]}'
        label = f'k_values {i+1}'
        
        ax.plot(T_fit_real/3600, L_Tau_fit_last[i,:], color=color, label=label)
